...
import { ShoppingCart, Warehouse, Tag, Settings, LogOut, ChevronRight } from 'lucide-react';
    ...
    <div className="space-y-5 animate-in">
      ...
      {user && (
        <Card className="p-4 flex items-center gap-4">
          <Avatar nome={user.nome} size={52} />
          <div className="flex-1">
            <p className="font-bold text-text-primary">{user.nome}</p>
            <p className="text-xs text-text-secondary">{user.role === 'admin' ? 'Administrador' : 'Leitor'}</p>
          </div>
        </Card>
      )}
      ...
      <button onClick={() => { logout(); router.push('/login'); toast.success('Até logo! 🙏'); }} className="w-full flex items-center justify-center gap-2 py-3 border border-error/40 text-error rounded-xl text-sm font-semibold hover:bg-red-50 transition-colors">
        <LogOut size={16} /> Sair
      </button>
